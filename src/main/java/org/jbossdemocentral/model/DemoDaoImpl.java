...
public class DemoDaoImpl implements DemoDao {
	...
	public DemoDaoImpl() {
		...
		if(this.githubApiClientId==null || "".equals(this.githubApiClientId)) {
			throw new RuntimeException("Missing environment variable " + GITHUB_API_CLIENT_ID_ENV_VAR);
		}
		if(this.githubApiClientId==null || "".equals(this.githubApiClientId)) {
			throw new RuntimeException("Missing environment variable " + GITHUB_API_SECRET_ENV_VAR);
		}
	}
	...
	public List<Demo> getDemos() {
		...
		ClientRequest request = new ClientRequest("https://api.github.com/orgs/jbossdemocentral/repos");
		...
		try {
			...
			for (Repository repo : repos) {
				...
				ClientRequest clientRequest = new ClientRequest(
						repo.getUrl() + "/contents/demo-config.json");
				...
				if (configFileResponse != null
					...
					Demo demo = mapper.readValue(Base64.decodeBase64(configFileResponse
							.getContent().getBytes()), Demo.class);
					demo.setId(repo.getId());
					log.info("Found demo with name " + demo.getName() + (demo.isPublished()?", that is pulblished":", that is NOT publised"));
					if(demo.isPublished())		
						list.add(demo);
				}
			}
		} catch (Exception e) {
			log.log(Level.WARNING,"Failed to parse response from the GitHub API");
			try {
				log.log(Level.WARNING," - Response status=" + request.get().getResponseStatus());
			} catch (Exception e1) { 
